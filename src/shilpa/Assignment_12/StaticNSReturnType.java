...
package shilpa.Assignment_12;

public class StaticNSReturnType {
	...
	boolean isEligible(double avg) {
		if (isAnswerAboveExpecation(avg) == true || avg % 2 == 0) {
			return true;
		} else
			return false;
	...
	public static void main(String[] args) {
		StaticNSReturnType st = new StaticNSReturnType();
		double average = st.average(80, 80, 90);
		if (st.isEligible(average) == true) {
			System.out.println("Candidate is Eligible");
		} else {
			System.out.println("Candidate is not eligible");
		}

		System.out.println("Candidate Obtained " + getYourGrade(average));
