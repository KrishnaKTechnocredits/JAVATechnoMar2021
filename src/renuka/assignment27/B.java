...
public class B extends A {
	...
	/*public static void main(String[] args) {
		...
		a1.classAM1();
		a1.M2();
		// a1.classBM3(); This will give CE Reason : Derived class(class B) properties
		// can't be accessible to parent class(Class A)

		System.out.println("");
		System.out.println("Case 2 OUTPUT--->");
		// All cases will work as Child Class(ClassB) will acquire all properties of
		// Parent Class(ClassA)
		B b1 = new B();
		...
		b1.classAM1();
		b1.M2(); // Exactly same method available in Parent class but priority give to method
					// available in derived class of which object is created
		b1.classBM3();

		System.out.println("");
		System.out.println("Case 3 OUTPUT--->");
		A a3 = new B(); // Example of Dynamic Polymorphism
		System.out.println(a3.x);
		System.out.println(a3.y); // it will consider x value from ClassA as property will not change in dynamic
									// polymorphism concept
		// System.out.println(a3.z); // This is CE - Reason : class A can only gurantee
		// for variables and Methods that are available in classA
		a3.classAM1();
		a3.M2(); // Here Class A m2 method is considered in compilation so no CE, but on run time
					// execution class B is called because behaviour will change at run time.
		// a3.classBM3(); //compiler will only look to left side and check if any method
		// present in ClassA
